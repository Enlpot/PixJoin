...
![Release](https://github.com/Enlpot/PixJoin/actions/workflows/release.yml/badge.svg)



![Version](https://img.shields.io/github/v/release/Enlpot/PixJoin)



![License](https://img.shields.io/github/license/Enlpot/PixJoin)
...
* **标注**：右键贴图 →「标注…」进入标注模式。矩形 / 椭圆 / 箭头 / 画笔 / 文字 / 高亮 / 马赛克 / 序号，8 色 + 3 档线宽；`Ctrl+Z` 撤销上一步，`Esc` 或 ✓ 完成固化（不降清晰度，可右键「撤销标注 / 处理」回退）

* **图像处理**：右键贴图 →「图像处理」：灰度 / 反色 / 模糊 / 锐化 / 旋转 90°180°270° / 水平垂直翻转，即时应用、可撤销
...
> `v0.2.0`
>
> ）时，GitHub Actions 会自动构建并同时发布以上两个版本，附 SHA256 校验文件。
...
| 截图后直接标注      | 选区下方点「标注」或按 `A`（贴图并进入标注模式）       |
